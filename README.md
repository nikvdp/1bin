...
1bin takes care of automating this process so that any package that is available on [conda-forge](https://conda-forge.org/) (and there are a lot of them!) can be converted into a single self-extracting binary for macOS or Linux. For now it’s `x86_64` only, but this approach could relatively easily be extended to build arm or mac M1 self-extracting executables in the same way, and with some work even support building Windows packages this way!
